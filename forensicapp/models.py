...
class Evidence(models.Model):
        ...
        accident = models.ForeignKey('Incident', on_delete=models.CASCADE, related_name='evidences' , null=True, blank=True)
        sample_type = models.CharField(max_length=255, null=True, blank=True)
        sample_number = models.CharField(max_length=255, null=True, blank=True)
        place_of_lifting = models.CharField(max_length=255, null=True, blank=True)
        method_lifting = models.CharField(max_length=255, null=True, blank=True)
        status = models.CharField(max_length=50, default='local')
...
class InspectionForm(models.Model):
    uuid = models.CharField(max_length=255, unique=True, blank=True, null=True)
    form = models.ForeignKey('Incident', on_delete=models.CASCADE, related_name='inspection_forms' , null=True, blank=True)
    is_chemistry_lab = models.BooleanField(default=False)
    is_weapons_lab = models.BooleanField(default=False)
    is_forensic_lab = models.BooleanField(default=False)
    is_criminal_print = models.BooleanField(default=False)
    is_dna_lab = models.BooleanField(default=False)
    is_criminal_electronic = models.BooleanField(default=False)
    status = models.CharField(max_length=50, default='local')
    user = models.ForeignKey(CustomUser, on_delete=models.SET_NULL, null=True, blank=True)
    created_at = models.DateTimeField(default=timezone.now)
    updated_at = models.DateTimeField(auto_now=True)
